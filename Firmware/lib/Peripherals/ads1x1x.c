...
void ADS1x1x_write_register(uint8_t i2c_address, uint8_t reg, uint16_t value)
{
  ...
  if (I2C_Write(i2c_address, Data, 3, false))
  {
    return;// (ADS1x1x_FAILED);
  }
...
*/
/**************************************************************************/
uint32_t ADS1x1x_read_register(uint8_t i2c_address, uint8_t reg)
{
  uint8_t Data[4];
  uint16_t result;
  ...
  if (I2C_Write(i2c_address, Data, 1, false))
  {
    return (ADS1x1x_FAILED);
  }
  // uint16_t result = 0;
  ...
  if (I2C_Read(i2c_address, Data, 2, false) == I2C_FAILED)
  {
    return (ADS1x1x_FAILED);
  }
  result = Data[1] + (Data[0] << 8);

  // For some unknown reson the chip returns 65535 when it should be 0 ????
  if(result == 65535)
    result = 0;
  return(result);
